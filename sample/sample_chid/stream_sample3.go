/*
 * Collection utility of Sample3 Struct
 *
...
func (self *Sample3Stream) Aggregate(fn func(Sample3, Sample3) Sample3) *Sample3Stream {
	result := Sample3StreamOf()
	self.ForEach(func(v Sample3, i int) {
		if i == 0 {
			result.Add(fn(Sample3{}, v))
		} else {
...
func (self *Sample3Stream) Select(fn func(Sample3) interface{}) interface{} {
	_array := make([]interface{}, 0, len(*self))
	for _, v := range *self {
...
func (self *Sample3Stream) Where(fn func(Sample3) bool) *Sample3Stream {
	result := Sample3StreamOf()
	for _, v := range *self {
		if fn(v) {
			result.Add(v)
...
func (self *Sample3Stream) WhereSlim(fn func(Sample3) bool) *Sample3Stream {
	result := Sample3StreamOf()
	caches := map[string]bool{}
	for _, v := range *self {
		key := fmt.Sprintf("%+v", v)
