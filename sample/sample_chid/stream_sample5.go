/*
 * Collection utility of Sample5 Struct
 *
...
func (self *Sample5Stream) Aggregate(fn func(Sample5, Sample5) Sample5) *Sample5Stream {
	result := Sample5StreamOf()
	self.ForEach(func(v Sample5, i int) {
		if i == 0 {
			result.Add(fn(Sample5{}, v))
		} else {
...
func (self *Sample5Stream) Select(fn func(Sample5) interface{}) interface{} {
	_array := make([]interface{}, 0, len(*self))
	for _, v := range *self {
...
func (self *Sample5Stream) Where(fn func(Sample5) bool) *Sample5Stream {
	result := Sample5StreamOf()
	for _, v := range *self {
		if fn(v) {
			result.Add(v)
...
func (self *Sample5Stream) WhereSlim(fn func(Sample5) bool) *Sample5Stream {
	result := Sample5StreamOf()
	caches := map[string]bool{}
	for _, v := range *self {
		key := fmt.Sprintf("%+v", v)
