/*
 * Collection utility of Sample2 Struct
 *
...
func (self *Sample2Stream) Aggregate(fn func(Sample2, Sample2) Sample2) *Sample2Stream {
	result := Sample2StreamOf()
	self.ForEach(func(v Sample2, i int) {
		if i == 0 {
			result.Add(fn(Sample2{}, v))
		} else {
...
func (self *Sample2Stream) Select(fn func(Sample2) interface{}) interface{} {
	_array := make([]interface{}, 0, len(*self))
	for _, v := range *self {
...
func (self *Sample2Stream) Where(fn func(Sample2) bool) *Sample2Stream {
	result := Sample2StreamOf()
	for _, v := range *self {
		if fn(v) {
			result.Add(v)
...
func (self *Sample2Stream) WhereSlim(fn func(Sample2) bool) *Sample2Stream {
	result := Sample2StreamOf()
	caches := map[string]bool{}
	for _, v := range *self {
		key := fmt.Sprintf("%+v", v)
