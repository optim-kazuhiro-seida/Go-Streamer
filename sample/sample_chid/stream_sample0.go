/*
 * Collection utility of Sample0 Struct
 *
...
func (self *Sample0Stream) Aggregate(fn func(Sample0, Sample0) Sample0) *Sample0Stream {
	result := Sample0StreamOf()
	self.ForEach(func(v Sample0, i int) {
		if i == 0 {
			result.Add(fn(Sample0{}, v))
		} else {
...
func (self *Sample0Stream) Select(fn func(Sample0) interface{}) interface{} {
	_array := make([]interface{}, 0, len(*self))
	for _, v := range *self {
...
func (self *Sample0Stream) Where(fn func(Sample0) bool) *Sample0Stream {
	result := Sample0StreamOf()
	for _, v := range *self {
		if fn(v) {
			result.Add(v)
...
func (self *Sample0Stream) WhereSlim(fn func(Sample0) bool) *Sample0Stream {
	result := Sample0StreamOf()
	caches := map[string]bool{}
	for _, v := range *self {
		key := fmt.Sprintf("%+v", v)
