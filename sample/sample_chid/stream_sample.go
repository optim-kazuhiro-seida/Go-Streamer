/*
 * Collection utility of Sample Struct
 *
...
func (self *SampleStream) Aggregate(fn func(Sample, Sample) Sample) *SampleStream {
	result := SampleStreamOf()
	self.ForEach(func(v Sample, i int) {
		if i == 0 {
			result.Add(fn(Sample{}, v))
		} else {
...
func (self *SampleStream) Select(fn func(Sample) interface{}) interface{} {
	_array := make([]interface{}, 0, len(*self))
	for _, v := range *self {
...
func (self *SampleStream) Where(fn func(Sample) bool) *SampleStream {
	result := SampleStreamOf()
	for _, v := range *self {
		if fn(v) {
			result.Add(v)
...
func (self *SampleStream) WhereSlim(fn func(Sample) bool) *SampleStream {
	result := SampleStreamOf()
	caches := map[string]bool{}
	for _, v := range *self {
		key := fmt.Sprintf("%+v", v)
